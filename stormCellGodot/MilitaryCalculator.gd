...
const BASE_DMG_RATIO=.1 # Given one solider, no other modifiers, has one tenth chance of killing someone a day
...
static var STANCE_MULT_TABLE = {
	...
	SCConstants.Stance.DEFENSIVE: {
		SCConstants.Stance.AGGRESSIVE: ATK,
...
static func calculate_one_day_battle(attacker: Army, defender: Army, region):
	var atk_dmg_mult=calculate_full_multiplier(attacker,defender, region)
	var def_dmg_mult=calculate_full_multiplier(defender,attacker, region)
	var dmg_to_atk = def_dmg_mult * defender.size * BASE_DMG_RATIO
	var dmg_to_def = atk_dmg_mult * attacker.size * BASE_DMG_RATIO 
	return [dmg_to_atk, dmg_to_def]
